...
export default function RecordList() {
  ...
  useEffect(() => {
    async function getRecords() {
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5050"}/record/`);
      if (!response.ok) {
  ...
  async function deleteRecord(id) {
    await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5050"}/record/${id}`, {
      method: "DELETE"
    });
